...
9. 화면 '내용'은 추측하지 마라(이 규칙은 화면 서술에만 해당하고, 손 동작 관찰과는 무관하다).
   화면이 꺼져 있거나 글자가 또렷이 읽히지 않으면 "모니터 화면이 꺼져 있다" 또는
   "화면에 글자가 보이나 판독 불가"라고 상태까지만 적는다 — 무슨 프로그램/OS/창인지
   이름을 지어내지 마라. "꺼져 있다"는 상태도 어엿한 관찰이니 반드시 기록한다.
