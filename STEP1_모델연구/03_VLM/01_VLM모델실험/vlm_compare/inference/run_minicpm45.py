...
class MiniCPMRunner:
    ...
    def load(self) -> None:
        ...
        # trust_remote_code 모델(InternVL/MiniCPM-V)의 _tied_weights_keys가 transformers 5.x의
        # 로딩 경로(all_tied_weights_keys)와 호환되지 않아 AttributeError가 났었다(실측 확인,
        # 2026-07-05). env_minicpm45의 transformers를 4.57.6으로 낮춰서 근본 원인을 해결했고,
        # 이제 Qwen3-VL과 동일하게 nf4 4bit 양자화를 다시 적용한다.
        self.quantization = "nf4 4bit (resampler 제외)"
        quant_config = BitsAndBytesConfig(
            load_in_4bit=True, bnb_4bit_quant_type="nf4",
            bnb_4bit_compute_dtype=torch.bfloat16, bnb_4bit_use_double_quant=True,
            # resampler는 nn.MultiheadAttention을 쓰는데, 이 모듈의 내부 경로가 4bit
            # Linear를 우회해서 바로 F.linear를 호출해 "Half and Byte" dtype 에러가 났다
            # (실측 확인, 2026-07-05). resampler만 양자화에서 제외해 우회한다.
            llm_int8_skip_modules=["resampler"],
        )
        print(f"[minicpm45] 모델 로딩 중... (attn_implementation={self.attn_implementation}, quant=nf4 4bit)")
        t0 = time.monotonic()
    ...
    try:
        if args.image:
            ...
            for image_path in frame_paths:
                ...
                out_path = output_dir / f"{video_id}_{frame_idx}.json"
                if out_path.exists():
                    print(f"[minicpm45] {video_id} frame_idx={frame_idx}: 이미 결과 있음, 스킵")
                    continue
                fdata = frames_by_idx.get(frame_idx, {"timestamp_sec": None, "detections": []})
                process_frame(runner, image_path, video_id, frame_idx, fdata["timestamp_sec"],
                              fdata["detections"], frame_width, frame_height,
