...
class InternVLRunner:
    ...
    def load(self) -> None:
        ...
        # 2026-07-05). env_internvl35의 transformers를 4.57.6으로 낮춰서 근본 원인을 해결했고,
        ...
        print(f"[internvl35] 모델 로딩 중... (attn_implementation={self.attn_implementation}, quant=nf4 4bit)")
        t0 = time.monotonic()
        ...
        self._model = AutoModel.from_pretrained(
            ...
            dtype=torch.bfloat16,  # 미양자화 서브모듈(vision tower 등)의 기본 dtype이 fp16이라
                                    # bf16으로 캐스팅한 pixel_values와 안 맞아 conv2d에서 dtype
                                    # 불일치 에러가 났다(실측 확인, 2026-07-05). 명시로 통일.
        ).eval()
    ...
    try:
        if args.image:
            ...
            for image_path in frame_paths:
                ...
                out_path = output_dir / f"{video_id}_{frame_idx}.json"
                if out_path.exists():
                    print(f"[internvl35] {video_id} frame_idx={frame_idx}: 이미 결과 있음, 스킵")
                    continue
                fdata = frames_by_idx.get(frame_idx, {"timestamp_sec": None, "detections": []})
                process_frame(runner, image_path, video_id, frame_idx, fdata["timestamp_sec"],
                              fdata["detections"], frame_width, frame_height,
