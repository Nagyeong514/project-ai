# 실행: conda run -n qwen-vl python try_5_최종코드.py [video_path] [output_dir]
# video_path: Motion Guide Sampling으로 이어붙인 영상 경로 (motion_selected.mp4)
...
# ────────────────────────────────────────────
# [수정 가능] 영상 → 부품명 매핑
# Motion Guide Sampling으로 이어붙인 영상 경로를 넣는다
# ────────────────────────────────────────────
VIDEOS = {
    "/home/piai/다운로드/퀸/output/motion_selected_RAM.mp4":  "RAM(DDR 메모리 모듈)",
    "/home/piai/다운로드/퀸/output/motion_selected_GPU.mp4":  "GPU(PCIe 그래픽카드)",
    # 영상 추가 시: "/경로/motion_selected_XXX.mp4": "부품명"
}

# ────────────────────────────────────────────
# [수정 가능] 경로 설정
# ────────────────────────────────────────────
DEFAULT_VIDEO  = "/home/piai/다운로드/퀸/output/motion_selected_RAM.mp4"
DEFAULT_OUTPUT = "/home/piai/다운로드/퀸/결과_try5"
...
# ────────────────────────────────────────────
# [수정 가능] 모델 설정
# Motion Guide Sampling 이어붙인 영상 기준 fps 설정
#   RAM 기준 5.5분 영상:
#     fps=0.5 → 약 165프레임 (OOM 가능)
#     fps=0.3 → 약  99프레임 (권장)
#     fps=0.2 → 약  66프레임 (OOM 발생 시)
# ────────────────────────────────────────────
model_name = "Qwen/Qwen3-VL-8B-Instruct"
SETTINGS = {
    "max_pixels":         192 * 192,
    "fps":                0.3,      # 이어붙인 영상(5~6분)은 0.3 권장, OOM 시 0.2로 낮출 것
    "max_new_tokens":     4000,
    "do_sample":          False,
...
[영상 구조 안내]
이 영상은 Motion Guide Sampling으로 선별된 핵심 작업 구간들을 이어붙인 영상이다.
구간 사이에 장면이 급격히 전환될 수 있으나, 모두 동일한 부품({PART}) 조립 작업의 일부다.
장면 전환이 발생해도 동작 분석을 중단하지 말고 계속 기록한다.
...
- 장면 전환 후 첫 행동은 이전 행동과의 연속성에 "장면 전환 후 시작"으로 표기한다.

──────────────────────────────
Step 4. 숙련자 암묵지 분析
──────────────────────────────

Step 3 행동 로그를 바탕으로 아래 항목을 분析한다.
...
4-B. 삽입 힘 프로파일
  - 아래 4단계 순서로 힘 변화를 기술한다:
    1단계 (접촉 직전): 속도 + 방향
    2단계 (첫 저항): 힘 크기(약/중/강) + 방향
    3단계 (저항 소멸): 순간 강 압력 여부
    4단계 (완료 후): 강→약 즉시 이완 또는 유지
  - 힘 전환 트리거: 소리·감각·시각 중 무엇인가?

4-C. 잠금 메커니즘 조작
  - 잠금 장치를 여는 동작과 잠기는 방식(자동/수동)을 분해하라.
  - 잠금 완료를 어떻게 인식하는가? (시각/촉각/청각)
...
user_prompt = USER_PROMPT_TEMPLATE.replace("{PART}", PART)
...
def save(result):
    SEP = "=" * 60
    save_path = os.path.join(output_dir, f"result_{video_name}_{ts}.txt")
    header = f"""영상 경로: {video_path}
지정 부품명: {PART}
분석 시각: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
모델: {model_name} (4bit NF4)
설정:
  max_pixels        = {SETTINGS['max_pixels']}  ({int(SETTINGS['max_pixels']**0.5)}x{int(SETTINGS['max_pixels']**0.5)})
  fps               = {SETTINGS['fps']}
  max_new_tokens    = {SETTINGS['max_new_tokens']}
  repetition_penalty= {SETTINGS['repetition_penalty']}
{SEP}
