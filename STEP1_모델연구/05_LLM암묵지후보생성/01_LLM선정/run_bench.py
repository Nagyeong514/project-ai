...
def main() -> None:
    ...
    # 2026-07-11: Qwen3(텍스트) 계열 hybrid thinking 차단 — <think> 블록이 draft 파싱을
    # 오염시키고 latency를 왜곡한다. STEP5 llm_fusion._load의 검증된 주입 패턴 그대로.
    # 기존 3종(no_think 미설정)은 코드 경로 불변.
    if mcfg.get("no_think"):
        _orig_act = tok.apply_chat_template
        ...
        print(f"[BENCH] {mcfg['repo_id']}: enable_thinking=False 주입")
