...
# 벡터DB 백엔드는 config.vector_backend에서 실제 선택된 것만 검사한다(CLAUDE.md 원칙4:
# 안 쓰는 백엔드 미설치를 오탐하지 않기 위함). 2026-07-14 Qdrant→Chroma 전환.
_BACKEND_MODULES = {
    "chroma": ["chromadb"],
...
def check_imports() -> list[str]:
    ...
    # config에서 선택된 벡터DB 백엔드의 패키지만 추가로 검사(원칙4).
    try:
        from config import CONFIG
        backend = getattr(CONFIG, "vector_backend", "chroma")
    except Exception:
        backend = "chroma"
    modules += _BACKEND_MODULES.get(backend, _BACKEND_MODULES["chroma"])
