...
[절대 규칙]
1. 관찰만 한다. 해석·추론·의도파악·진단 결론은 절대 하지 않는다(그건 다음 단계의 일).
   연속된 동일·동종 동작은 하나로 묶어 적어도 된다. 단, 서로 다른 동작을 억지로 하나로 뭉치지는 마라.
...
5. 전원 버튼 LED는 색과 깜빡임 '횟수'만 "<색상>으로 N회 깜빡인다" 형식으로 적는다.
   ...
   전원 버튼 LED 색은 황색(amber) 또는 백색(white) 중 실제로 보이는 색으로만 기록하라.
   빨강 등 다른 색으로 보이면 광원 반사일 수 있으니, 확실하지 않으면 색을 단정하지 말고
   "점멸이 보이나 색 판독 불가"라고 적어라.
...
8. **같은 문장을 두 번 쓰지 마라.** 동일한 동작이 여러 프레임에 걸쳐 계속되면 처음 한 번만
   적고 넘어간다. 동작이 눈에 띄게 달라졌을 때만 새 entry를 만든다.
   비슷한 동작이 다시 나타나면 무엇이 달라졌는지(다른 부품, 다른 위치, 다른 손)를 문장에 담아라.
9. 화면이 꺼져 있거나 판독 불가한 구간에서는 화면 내용을 추측해 채우지 마라.
   또렷이 읽히는 글자가 없으면 "모니터 화면이 꺼져 있다" 또는 "화면에 글자가 보이나 판독 불가"
   까지만 적고, 무슨 프로그램/OS/창인지 추측하지 마라. 확실치 않으면 그 항목을 만들지 마라.
   (규칙 7의 화면 상태 관찰 자체는 계속 한다 — "꺼져 있다"는 상태도 어엿한 관찰이다.)

[출력 형식 — 이 JSON만 출력. 배열 밖 텍스트 절대 금지]
{
  "observations": [
    {
      "timestamp": "<HH:MM:SS 형식 시각>",
...
위에 정의된 4개 필드(timestamp, actor, action, objects_visible) 외에 raw 등 임의 필드를 만들지 마라.
"""
...
def build_video_observation_messages(
    ...
    blocks.append(
        ...
        "각 observation 의 timestamp 는 해당 장면의 시각을 HH:MM:SS 형식으로 적는다"
        "(기준·범위는 뒤에 오는 구간 안내를 따른다).\n"
