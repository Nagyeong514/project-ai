...
TACIT_DIR = PROJECT_ROOT / "STEP5_LLM_암묵지_후보생성" / "output" / "tacit_json"  # 2026-07-14: 폴더명 드리프트 수정('LLM으로'→'LLM')
...
def run(output_root: Path, tacit_dir: Path = TACIT_DIR) -> Dict[str, int]:
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--output", default=str(PROJECT_ROOT / "STEP5_LLM_암묵지_후보생성" / "output" /
                                 "step6_adapter_input" / "motion_run1"),
        help="STEP6 --input 에 그대로 넘길 폴더 경로"
    )
    ap.add_argument(
        "--tacit-dir", default=str(TACIT_DIR),
        help="변환할 STEP5 tacit_json 폴더(기본: 본선 output/tacit_json)"
    )
    args = ap.parse_args()
    out_dir = Path(args.output)
    stats = run(out_dir, tacit_dir=Path(args.tacit_dir))
