...
STEP6_품질검증/preflight.py와 동일 원칙(docs/실행전_방어_체크리스트.md 참고).

⚠️ 이 파일 자체는 표준 라이브러리(importlib/os/sys/glob)만 쓴다 — "패키지 없는지
...
def run_preflight() -> None:
    """build_db.py의 진입 함수 맨 첫 줄에서 호출한다. 실패하면 즉시 sys.exit(1)."""
    steps = [
...
if __name__ == "__main__":
    # 단독 실행: 본 실행(build_db.py) 전에 값싼 자원으로 미리 확인만 하고 싶을 때.
    #   .venv/bin/python3 preflight.py   (device=cpu 기본값이면 GPU 할당 불필요)
    run_preflight()
