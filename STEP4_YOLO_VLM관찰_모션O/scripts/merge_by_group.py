...
모션O run_step4.py는 클립 파일 하나당 산출물 하나를 만든다. 이 스크립트는 같은 그룹의
...
사용:
    python scripts/merge_by_group.py            # output/*_by_group/ 에 그룹당 1파일 생성
"""
...
def main() -> None:
    frames_dir = MO_ROOT / "output" / "_frames"
    obs_dir = MO_ROOT / "output" / "vlm_observations"
    det_dir = MO_ROOT / "output" / "detections"
    obs_out = MO_ROOT / "output" / "vlm_observations_by_group"
    det_out = MO_ROOT / "output" / "detections_by_group"
    ...
    if not groups:
        sys.exit("병합할 산출물이 없음 — run_step4.py 를 먼저 돌릴 것")
